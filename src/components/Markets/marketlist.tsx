...
import ItemCardMarket from "./ItemCardMarket";
import { handleMissingColors } from "@/utils";
import { useInView } from "framer-motion";
import { useRef } from "react";
import {motion} from 'framer-motion'
import { AppearFromBottom, ImageFromLeft } from "@/AnimationUtils";
export default function marketlist({blok}:{blok:any}){
  ...
  const {anchor_id}=blok;
    ...
    return(
        <div className=" flex flex-col h-auto px-[1em] md:px-[3em] xl:px-[5em] py-[4em] md:py-[6em] lg:py-[8em]" ref={ref} style={{backgroundColor:colors[0].background_color,color:colors[0].text_color}} id={anchor_id} {...storyblokEditable(blok)}>
          ...
          <div className=' grid grid-cols-1 md:grid-cols-2 1400px:grid-cols-4 w-full justify-center gap-[20px] lg:px-[10px] mt-[30px] lg:mt-[65px] max-w-[1752px] m-auto'>
             
                <ItemCardMarket isInView={isInView} divider_color={card_divider_color} link_variant={colors[0].link_variant} buttontext={buttontext} blok={markets}/>
